...
class HubStaffOAuth(OAuth):
    ...
    def get_code_url(redirect_uri: str, state: dict) -> str:
        ...
        :param redirect_uri: Ссылка на WebHook куда придет уведомление об успешной автоизации клиента.
        :param state: Данные которые вернуться на WebHook в шифровоном base64.
        """
    ...
    def get_token_url(code: str, redirect_uri: str = None):
        """
        Генерирование ссылки на получение Токена авторизации.

        Для допольнительной информации по get параметрам https://developer.hubstaff.com/authentication <br>
        :param code Код подтверждения который приходит на WebHook после успешного входа.
        :param redirect_uri: Ссылка на WebHook куда придет уведомление об успешной автоизации клиента.
        """
